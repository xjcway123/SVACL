...
from extract_features import Vocab,gen_tok_pattern,extract_features
from transformers import AutoTokenizer
...
class GraphDataset_Token_MultiTask(Dataset):
    ...
    def __init__(self,  context_type: str, token_type : str = 'codebert' , partition="train"):
        ...
        datafile = "E:\\xjc\\SOTitlePlus\\baselines\\FVA-master\\FVA-master\\Data\\new_data.xlsx"
        ...
        self.vocab = None
        assert token_type in ["codebert","unixcoder","textcnn","lstm"]
        ...
        elif self.token_type in ["textcnn","lstm"]:
...
def feature_extraction(filepath,codebert_tokenizer:RobertaTokenizerFast,vocab:Vocab,token_type:str):
    ...
    pdg_nodes["nature_context"] = pdg_nodes.id.map(lambda x:"\n".join(code[i] for i in [x-1,x,x+1] if i in code.keys()))
