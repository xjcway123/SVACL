...
# construct soft template
from openprompt.prompts import ManualTemplate,MixedTemplate,SoftTemplate

#mix
template_text = '{"soft":"The code snippet:"} {"placeholder":"text_a"} {"soft":"The vulnerability description:"} {"placeholder":"text_b"} {"soft":"Classify the severity:"} {"mask"}'
...
for i in range(1, 6):
    ...
    if i >= 2:
        prompt_model.load_state_dict(
            torch.load(os.path.join("E:\\xjc\\SOTitlePlus\\SOTitlePlus\\model_code\\resultsoft\\best.ckpt"),
                       map_location=torch.device('cuda:0')))
    early_stop_count = 0
    prompt_model.train()

    # Train and validate the model
    output_dir = "resultsoft"
